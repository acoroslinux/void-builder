...
# Setup default root/user password (root/root and <live>/live).
# Use a SHA512 attempt first, then fallback to default chpasswd behaviour.
chroot "${NEWROOT}" sh -c 'echo "root:root" | chpasswd -c SHA512 || echo "root:root" | chpasswd'
chroot "${NEWROOT}" sh -c 'echo "$1:void" | chpasswd -c SHA512 || echo "$1:void" | chpasswd' _ "${USERNAME}"
chroot "${NEWROOT}" passwd -u "${USERNAME}" >/dev/null 2>&1 || true
