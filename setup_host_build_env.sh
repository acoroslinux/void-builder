...
# 7. Expose bundled xbps-static binaries on the system PATH
# -------------------------------------------------------
# The void-builder ships its own statically-linked xbps binaries inside
# void_builder/tools/usr/bin/.  On a fresh host they are not on PATH, which
# means scripts and manual invocations outside the Python process would fail.
# We register the directory via /etc/profile.d/ so every future shell session
# picks them up automatically.

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
XBPS_TOOLS_BIN="${SCRIPT_DIR}/void_builder/tools/usr/bin"
PROFILE_SNIPPET="/etc/profile.d/void-builder-xbps.sh"

echo ""
echo "==> Registering xbps-static tools in PATH..."
echo "    Directory: ${XBPS_TOOLS_BIN}"

cat > "${PROFILE_SNIPPET}" << SNIPPET_EOF
# void-builder: add bundled xbps-static binaries to PATH
# Generated by setup_host_build_env.sh — do not edit manually.
if [ -d "${XBPS_TOOLS_BIN}" ]; then
    case ":${PATH}:" in
        *":${XBPS_TOOLS_BIN}:"*) ;;
        *) export PATH="${XBPS_TOOLS_BIN}:\${PATH}" ;;
    esac
fi
SNIPPET_EOF
...
echo "    ✅ Created ${PROFILE_SNIPPET}"
echo ""
echo "  To apply immediately in this shell session, run:"
echo "    source ${PROFILE_SNIPPET}"
echo ""
echo "  (All new login/interactive shells will pick it up automatically.)"
